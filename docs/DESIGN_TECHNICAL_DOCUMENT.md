...
自动化测试覆盖 Agent、策略、工具、确认、加密、并发、审计篡改、MCP 生命周期与 Schema、Web 认证、配置、安装资源和前端逻辑。当前 216 项 Python 测试与 7 项 Node 测试全部通过；Python 分支覆盖综合值为 72.1%，CI 门槛为 70.0%，且以 ResourceWarning 作为错误运行。
